...
(function () {
  ...
  async function calculateMemberXp(user) {
    ...
    try {
      ...
      const commentLikeResult = calculateCommentReceivedLikeXp(comments);

      const totalXp =
        postResult.postXp +
        receivedLikeResult.articleReceivedLikeXp +
        givenLikeResult.articleGivenLikeXp +
        commentLikeResult.commentLikeXp;
      ...
      return {
        ...
        commentLikeXp: commentLikeResult.commentLikeXp,
        totalCommentLikes: commentLikeResult.totalCommentLikes
      };
    } catch (error) {
  ...
  function renderProfileContent(displayName, levelInfo, xpInfo) {
    ...
    if (xpSourceEl) {
      xpSourceEl.innerHTML = `
        ...
        <div>留言獲讚 XP：${xpInfo.commentLikeXp}</div>
      `;
    }
  }
  ...
  window.BGCProfile = {
    ...
    getXpRequiredForLevel: getXpRequiredForLevel
  };
